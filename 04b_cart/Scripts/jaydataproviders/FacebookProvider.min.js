...
or:{mapTo:" OR ",dataType:$data.Boolean,allowedIn:$data.Expressions.FilterExpression},and:{mapTo:" AND ",dataType:$data.Booleanv},"in":{mapTo:" IN ",dataType:$data.Boolean,resolvableType:[$data.Array,$data.Queryable],allowedIn:$data.Expressions.FilterExpression}}},supportedUnaryOperators:{value:{}},fieldConverter:{value:{fromDb:{"$data.Number":function(b){return"number"===typeof b?b:parseInt(b)},"$data.Integer":function(b){return"number"===typeof b?b:parseFloat(b)},"$data.String":function(b){return b},
"$data.Date":function(b){return new Date("string"===typeof b?parseInt(b):b)},"$data.Boolean":function(b){return!!b},"$data.Blob":function(b){return b},"$data.Array":function(b){return void 0===b?new $data.Array:b}},toDb:{"$data.Number":function(b){return b},"$data.Integer":function(b){return b},"$data.String":function(b){return"'"+b+"'"},"$data.Date":function(b){return b?b.valueOf():null},"$data.Boolean":function(b){return b},"$data.Blob":function(b){return b},"$data.Array":function(b){return"("+
b.join(", ")+")"}}}},supportedSetOperations:{value:{filter:{},map:{},forEach:{},toArray:{},single:{},take:{},skip:{},orderBy:{},orderByDescending:{},first:{}},enumerable:!0,writable:!0},executeQuery:function(b,a){a=$data.typeSystem.createCallbackSetting(a);this.AuthenticationProvider||(this.AuthenticationProvider=new $data.Authentication.Anonymous({}));var c;try{c=this._compile(b)}catch(e){a.error(e);return}var d=b.defaultType,f=[];c.selectMapping||this._discoverType("",d,f);c={url:this.providerConfiguration.FQLQueryUrl+
encodeURIComponent(c.queryText)+"&"+this.providerConfiguration.FQLFormat,dataType:"JSON",success:function(c){b.rawDataList=c.data;Container.createModelBinderConfigCompiler(b,[]).Visit(b.expression);a.success(b)},error:function(b,c,e){c={};try{c=JSON.parse(b.responseText).error}catch(f){c=e+": "+b.responseText}a.error(c)}};this.context.prepareRequest.call(this,c);this.AuthenticationProvider.CreateRequest(c)},_discoverType:function(b,a,c){a.memberDefinitions.getPublicMappedProperties().forEach(function(a){var d=
...
c).expression,a=Container.createQueryExpressionCreator(b.context).Visit(a),b={projectionSql:{sql:""}};this.Visit(a,b);return b.projectionSql},generateProjectionFunc:function(b){var a=this.provider.AuthenticationProvider.Authenticated,b=b.defaultType.memberDefinitions.getPublicMappedProperties();!a&&b.some(function(a){return!0==a.isPublic})&&(b=b.filter(function(a){return!0==a.isPublic}));var c="function (s){ return {";b.forEach(function(a,b){0!=b&&(c+=", ");c+=a.name+": s."+a.name});return c+="};"},
VisitFilterExpression:function(b,a){this.Visit(b.source,a);a.filterSql.type=b.nodeType;a.filterSql.sql=""==a.filterSql.sql?" WHERE ":a.filterSql.sql+" AND ";this.Visit(b.selector,a.filterSql)},VisitProjectionExpression:function(b,a){this.Visit(b.source,a);a.projectionSql.type=b.nodeType;""==a.projectionSql.sql?a.projectionSql.sql="SELECT ":Guard.raise(new Exception("Multiple select error"));this.Visit(b.selector,a.projectionSql)},VisitOrderExpression:function(b,a){this.Visit(b.source,a);a.orderSql.type=
b.nodeType;""==a.orderSql.sql?a.orderSql.sql=" ORDER BY ":Guard.raise(new Exception("Multiple sorting not supported","not supported"));this.Visit(b.selector,a.orderSql);a.orderSql.sql+=b.nodeType==$data.Expressions.ExpressionType.OrderByDescending?" DESC":" ASC"},VisitPagingExpression:function(b,a){this.Visit(b.source,a);b.nodeType==$data.Expressions.ExpressionType.Skip?(a.skipSql.type=b.nodeType,a.skipSql.sql=" OFFSET ",this.Visit(b.amount,a.skipSql)):b.nodeType==$data.Expressions.ExpressionType.Take&&
(a.takeSql.type=b.nodeType,a.takeSql.sql=" LIMIT ",this.Visit(b.amount,a.takeSql))},VisitSimpleBinaryExpression:function(b,a){a.sql+="(";this.Visit(b.left,a);a.sql+=b.resolution.mapTo;b.resolution.resolvableType&&!Guard.requireType(b.resolution.mapTo+" expression.right.value",b.right.value,b.resolution.resolvableType)&&Guard.raise(new Exception(b.right.type+" not allowed in '"+b.resolution.mapTo+"' statement","invalid operation"));this.Visit(b.right,a);a.sql+=")"},VisitEntityFieldExpression:function(b,
a){this.Visit(b.selector,a)},VisitMemberInfoExpression:function(b,a){var c=b.memberName;a.sql+=c;a.fieldData={name:c,dataType:b.memberDefinition.dataType};"Projection"==a.type&&!a.selectFields&&(a.selectFields=!0===a.fieldOperation?[{from:"anon"}]:[{from:c,dataType:b.memberDefinition.dataType}])},VisitConstantExpression:function(b,a){"Projection"==a.type&&Guard.raise(new Exception("Constant value is not supported in Projection.","Not supported!"));this.VisitQueryParameterExpression(b,a)},VisitQueryParameterExpression:function(b,
a){var c=Container.resolveType(Container.getTypeName(b.value));if(-1!=this.provider.supportedDataTypes.indexOf(c))a.sql+=this.provider.fieldConverter.toDb[Container.resolveName(c)](b.value);else switch(c){case $data.Queryable:a.sql+="("+b.value.toTraceString().queryText+")";break;default:a.sql+=""+b.value+""}},VisitParametricQueryExpression:function(b,a){this.Visit(b.expression,a);a.parameters=b.parameters},VisitEntitySetExpression:function(b,a){a.tableName=b.instance.tableName},VisitObjectLiteralExpression:function(b,
a){var c=this;a.selectFields=a.selectFields||[];b.members.forEach(function(b){b.expression instanceof $data.Expressions.ObjectLiteralExpression?(a.mappingPrefix=a.mappingPrefix||[],a.mappingPrefix.push(b.fieldName),c.Visit(b,a),a.mappingPrefix.pop()):(0<a.selectFields.length&&(a.sql+=", "),c.Visit(b,a),b=a.mappingPrefix instanceof Array?a.mappingPrefix.join(".")+"."+b.fieldName:b.fieldName,a.selectFields.push({from:a.fieldData.name,to:b,dataType:a.fieldData.dataType}))})},VisitObjectFieldExpression:function(b,
a){return this.Visit(b.expression,a)},VisitEntityFieldOperationExpression:function(b,a){Guard.requireType("expression.operation",b.operation,$data.Expressions.MemberInfoExpression);var c=b.operation.memberDefinition,e=c.mapTo||c.name;a.sql+="(";!1==c.expressionInParameter&&this.Visit(b.source,a);a.sql+=e;a.sql+="(";var d=0;(c.parameters||[]).map(function(a){var c={dataType:a.dataType};a.value?c.value=a.value:"@expression"===a.name?c.value=b.source:(c.value=b.parameters[d],c.itemType=b.parameters[d++].type);
return c}).forEach(function(c,d){var e=c.itemType?Container.resolveType(c.itemType):null;if(!e||c.dataType instanceof Array&&-1!=c.dataType.indexOf(e)||c.dataType==e){if(0<d&&(a.sql+=", "),"Projection"==a.type&&(a.fieldOperation=!0),this.Visit(c.value,a),"Projection"==a.type)a.fieldOperation=void 0}else Guard.raise(new Exception(parameter.type+" not allowed in '"+b.operation.memberName+"' statement","invalid operation"))},this);a.fieldData&&a.fieldData.name&&(a.fieldData.name="anon");a.sql=c.rigthValue?
a.sql+c.rigthValue:a.sql+")";a.sql+=")"}},null);
...
isPublic:!0},birthday:{type:"string",isPublic:!0},personal_info:{type:"string",isPublic:!0},personal_interests:{type:"string",isPublic:!0},built:{type:"string",isPublic:!0},features:{type:"string",isPublic:!0},mpg:{type:"string",isPublic:!0}},null);$data.Class.define("$data.storageProviders.Facebook.EntitySets.Command",null,null,{constructor:function(b){this.Config=$data.typeSystem.extend({CommandValue:""},b)},toString:function(){return this.Config.CommandValue},Config:{}},null);
$data.Class.define("$data.Facebook.FQLContext",$data.EntityContext,null,{constructor:function(){var b=this.Friends.where(function(a){return a.uid1==this.me},{me:$data.Facebook.FQLCommands.me}).select(function(a){return a.uid2});this.MyFriends=this.Users.where(function(a){return a.uid==this.me||a.uid in this.friends},{me:$data.Facebook.FQLCommands.me,friends:b})},Users:{dataType:$data.EntitySet,tableName:"user",elementType:$data.Facebook.types.FbUser},Friends:{dataType:$data.EntitySet,tableName:"friend",
