...
var g,h;switch(d){case "contains":g="$regex";h=a.value;break;case "startsWith":g="$regex";h="^"+a.value;break;case "endsWith":g="$regex",h=a.value+"$"}g&&h&&(a.cursor instanceof Array?(c={},c[a.field]={},c[a.field][g]=h,a.cursor.push(c)):(a.cursor[a.field]={},a.cursor[a.field][g]=h))},VisitConstantExpression:function(b,a){var c=Container.getTypeName(b.value);a.valueType=c;a.value=b.value},VisitEntityExpression:function(b,a){a.entityType=b.entityType;this.Visit(b.source,a)},VisitEntitySetExpression:function(b,
a){this.Visit(b.source,a);b.selector instanceof $data.Expressions.AssociationInfoExpression&&(this.Visit(b.selector,a),a.data+="/")},VisitFrameOperationExpression:function(b,a){this.Visit(b.source,a);Guard.requireType("expression.operation",b.operation,$data.Expressions.MemberInfoExpression);var c=b.operation.memberDefinition;a.data+=c.mapTo||c.name;a.data+="(";for(var d=0,f=(c.parameters||[{name:"@expression"}]).map(function(a){return"@expression"===a.name?b.source:b.parameters[d++]}),g=0;g<f.length;g++){var h=
f[g];if(h.value instanceof $data.Queryable){var e=new c.frameType(h.value.expression),h=(new $data.Expressions.QueryExpressionCreator(h.value.entityContext)).Visit(e),e={data:""};(new $data.storageProviders.mongoDB.mongoDBWhereCompiler(this.provider,!0)).compile(h,e);a.data+=e.lambda+": "+e.data}}a.data+=")"}});
...
c&&f(a,d)}):f(c,d)}})},executeQuery:function(b,a){var c=this,a=$data.typeSystem.createCallbackSetting(a),d=b.context.getEntitySetFromElementType(b.defaultType);(new $data.storageProviders.mongoDB.mongoDBCompiler).compile(b);(new this.driver.Db(this.providerConfiguration.databaseName,this._getServer(),{})).open(function(f,g){if(f)a.error(f);else{var h=new c.driver.Collection(g,d.tableName),e=b.find,n=function(d,e){d?a.error(d):(b.rawDataList=e instanceof Array?e:[{cnt:e}],b.context=c.context,a.success(b),
g.close())},r=function(){switch(b.expression.nodeType){case $data.Expressions.ExpressionType.Count:h.find(e.query,e.options).count(n);break;case $data.Expressions.ExpressionType.BatchDelete:h.remove(e.query,{safe:!0},n);break;default:h.find(e.query,e.options).toArray(n)}};c.providerConfiguration.username?g.authenticate(c.providerConfiguration.username,c.providerConfiguration.password,function(b,c){b?a.error(b):c&&r()}):r()}})},_typeFactory:function(b,a,c){b=Container.resolveName(b);return c&&c[b]?
c[b](a):new (Container.resolveType(b))(a)},_saveCollections:function(b,a){var c=this,d=0,f=this._getServer(),g=0,h=function(a){0==--g&&a()},e=function(a,e,g){for(var f=[],h=0;h<e.insertAll.length;h++){for(var m=e.insertAll[h],o=Container.resolveType(m.type).memberDefinitions.getPublicMappedProperties(),k=0;k<o.length;k++){var i=o[k];if(i.concurrencyMode===$data.ConcurrencyMode.Fixed)m.data[i.name]=0;else if(i.computed)"string"===typeof m.data[i.name]&&(m.data._id=c._typeFactory(i.type,m.data[i.name],
c.fieldConverter.toDb));else{if(Container.resolveType(i.type)===$data.Array&&i.elementType&&Container.resolveType(i.elementType)===$data.ObjectID){m.data[i.name]=c._typeFactory(i.type,m.data[i.name],c.fieldConverter.toDb);var q=m.data[i.name];if(q)for(var l=0;l<q.length;l++)q[l]=c._typeFactory(i.elementType,q[l],c.fieldConverter.toDb)}else m.data[i.name]=c._typeFactory(i.type,m.data[i.name],c.fieldConverter.toDb);m.data[i.name]&&m.data[i.name].initData&&(m.data[i.name]=m.data[i.name].initData)}}f.push(m.data)}g.insert(f,
{safe:!0},function(h,i){if(h)b.error(h);else{for(var f=0;f<i.length;f++)for(var m=i[f],k=e.insertAll[f],l=Container.resolveType(k.type).memberDefinitions.getPublicMappedProperties(),p=0;p<l.length;p++){var j=l[p];k.entity[j.name]=c._typeFactory(j.type,m[j.computed?"_id":j.name],c.fieldConverter.fromDb)}d+=i.length;e.removeAll&&e.removeAll.length?r(a,e,g):e.updateAll&&e.updateAll.length?n(a,e,g):s(a,d)}})},n=function(a,e,f){g=e.updateAll.length;for(var p=0;p<e.updateAll.length;p++){for(var j=e.updateAll[p],
m={},o=Container.resolveType(j.type).memberDefinitions.getKeyProperties(),k=0;k<o.length;k++){var i=o[k];m[i.computed?"_id":i.name]=c.fieldConverter.toDb[Container.resolveName(Container.resolveType(i.type))](j.entity[i.name])}for(var q={},o=Container.resolveType(j.type).memberDefinitions.getPublicMappedProperties(),k=0;k<o.length;k++){var l=o[k];if(l.concurrencyMode===$data.ConcurrencyMode.Fixed)m[l.name]=c._typeFactory(l.type,j.entity[l.name],c.fieldConverter.toDb),q.$inc||(q.$inc={}),q.$inc[l.name]=
1;else if(!l.computed&&"undefined"!==typeof j.entity[l.name])if(Container.resolveType(l.type)===$data.Array&&l.elementType&&Container.resolveType(l.elementType)===$data.ObjectID){q[l.name]=c._typeFactory(l.type,j.entity[l.name],c.fieldConverter.toDb);var n=q[l.name];if(n)for(i=0;i<n.length;i++)n[i]=c._typeFactory(l.elementType,n[i],c.fieldConverter.toDb)}else q[l.name]=c._typeFactory(l.type,j.entity[l.name],c.fieldConverter.toDb)}(function(e){f.update(m,{$set:q},{safe:!0},function(i,k){if(i)b.error(i);
else if(k){d++;for(var l=Container.resolveType(e.type).memberDefinitions.getPublicMappedProperties(),j=0;j<l.length;j++){var p=l[j];p.concurrencyMode===$data.ConcurrencyMode.Fixed&&e.entity[p.name]++}h(function(){s(a,d)})}else g--,f.find({_id:m._id},{}).toArray(function(f,i){if(f)b.error(f);else{for(var g=i[0],k=Container.resolveType(e.type).memberDefinitions.getPublicMappedProperties(),l=0;l<k.length;l++){var j=k[l];e.entity[j.name]=c._typeFactory(j.type,g[j.computed?"_id":j.name],c.fieldConverter.fromDb)}h(function(){s(a,
d)})}})})})(j)}},r=function(a,e,f){g=e.removeAll.length;for(var p=0;p<e.removeAll.length;p++){for(var j=e.removeAll[p],m=Container.resolveType(j.type).memberDefinitions.getKeyProperties(),o=0;o<m.length;o++){var k=m[o];j.data[k.computed?"_id":k.name]=c.fieldConverter.toDb[Container.resolveName(Container.resolveType(k.type))](j.entity[k.name])}m=Container.resolveType(j.type).memberDefinitions.getPublicMappedProperties();for(o=0;o<m.length;o++)k=m[o],k.computed||(j.data[k.name]=c.fieldConverter.toDb[Container.resolveName(Container.resolveType(k.type))](j.data[k.name]),
"undefined"===typeof j.data[k.name]&&delete j.data[k.name]),k.concurrencyMode!==$data.ConcurrencyMode.Fixed&&delete j.data[k.name];f.remove(j.data,{safe:!0},function(c,j){c?b.error(c):(j?d++:g--,h(function(){e.updateAll&&e.updateAll.length?n(a,e,f):s(a,d)}))})}},t=Object.keys(a),s=function(d,f){if(t.length){var g=t.pop();if(a.hasOwnProperty(g)){var h=a[g],g=new c.driver.Collection(d,g);h.insertAll&&h.insertAll.length?e(d,h,g):h.removeAll&&h.removeAll.length?r(d,h,g):h.updateAll&&h.updateAll.length?
n(d,h,g):(b.success(0),d.close())}}else b.success(f),d.close()};(new this.driver.Db(this.providerConfiguration.databaseName,f,{})).open(function(a,d){a?b.error(a):c.providerConfiguration.username?d.authenticate(c.providerConfiguration.username,c.providerConfiguration.password,function(a,c){a?b.error(a):c&&s(d)}):s(d)})},saveChanges:function(b,a){if(a.length){for(var c=this.buildIndependentBlocks(a),d=[],f={},g=0;g<c.length;g++)for(var h=0;h<c[g].length;h++){d.push(c[g][h].data);var e=f[c[g][h].entitySet.name];
e||(e={},f[c[g][h].entitySet.name]=e);var n={entity:c[g][h].data,data:this.save_getInitData(c[g][h],d),type:Container.resolveName(c[g][h].data.getType())};switch(c[g][h].data.entityState){case $data.EntityState.Unchanged:continue;case $data.EntityState.Added:e.insertAll||(e.insertAll=[]);e.insertAll.push(n);break;case $data.EntityState.Modified:e.updateAll||(e.updateAll=[]);e.updateAll.push(n);break;case $data.EntityState.Deleted:e.removeAll||(e.removeAll=[]);e.removeAll.push(n);break;default:Guard.raise(new Exception("Not supported Entity state"))}}this._saveCollections(b,
f)}else b.success(0)},save_getInitData:function(b,a){b.physicalData=this.context._storageModel.getStorageModel(b.data.getType()).PhysicalType.convertTo(b.data,a);var c={};b.physicalData.getType().memberDefinitions.asArray().forEach(function(a){if(a.kind==$data.MemberTypes.navProperty||a.kind==$data.MemberTypes.complexProperty||a.kind==$data.MemberTypes.property&&!a.notMapped)Container.resolveType(a.type)===$data.Array&&a.kind===$data.MemberTypes.property&&b.physicalData[a.name]?c[a.name]=JSON.parse(JSON.stringify(b.physicalData[a.name])):
c[a.computed?"_id":a.name]=b.physicalData[a.name]},this);return c},supportedDataTypes:{value:[$data.Integer,$data.String,$data.Number,$data.Blob,$data.Boolean,$data.Date,$data.ObjectID,$data.Object,$data.Geography],writable:!1},supportedBinaryOperators:{value:{equal:{mapTo:":",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},notEqual:{mapTo:"$ne",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},equalTyped:{mapTo:":",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},
notEqualTyped:{mapTo:"$ne",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},greaterThan:{mapTo:"$gt",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},greaterThanOrEqual:{mapTo:"$gte",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},lessThan:{mapTo:"$lt",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},lessThenOrEqual:{mapTo:"$lte",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},or:{mapTo:"$or",dataType:"boolean",
allowedIn:[$data.Expressions.FilterExpression]},and:{mapTo:"$and",dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression]},"in":{mapTo:"$in",allowedIn:[$data.Expressions.FilterExpression]}}},supportedUnaryOperators:{value:{not:{mapTo:"$nor"}}},supportedFieldOperations:{value:{contains:{dataType:"boolean",allowedIn:[$data.Expressions.FilterExpression],parameters:[{name:"substring",dataType:"string"}]},startsWith:{dataType:"string",allowedIn:[$data.Expressions.FilterExpression],parameters:[{name:"@expression",
dataType:"string"},{name:"strFragment",dataType:"string"}]},endsWith:{dataType:"string",allowedIn:[$data.Expressions.FilterExpression],parameters:[{name:"@expression",dataType:"string"},{name:"strFragment",dataType:"string"}]}},enumerable:!0,writable:!0},supportedSetOperations:{value:{filter:{},map:{},length:{},forEach:{},toArray:{},batchDelete:{},single:{},take:{},skip:{},orderBy:{},orderByDescending:{},first:{}},enumerable:!0,writable:!0},fieldConverter:{value:{fromDb:{"$data.Integer":function(b){return b},
"$data.Number":function(b){return b},"$data.Date":function(b){return b?new Date(b):b},"$data.String":function(b){return b},"$data.Boolean":function(b){return b},"$data.Blob":function(b){return b},"$data.Object":function(b){return void 0===b?new $data.Object:b},"$data.Array":function(b){return void 0===b?new $data.Array:b},"$data.ObjectID":function(b){return b?(new Buffer(b.toString(),"ascii")).toString("base64"):b},"$data.Geography":function(b){return b?new $data.Geography(b[0],b[1]):b}},toDb:{"$data.Integer":function(b){return b},
"$data.Number":function(b){return b},"$data.Date":function(b){return b},"$data.String":function(b){return b},"$data.Boolean":function(b){if("string"===typeof b)switch(b){case "true":case "true":case "TRUE":case "yes":case "Yes":case "YES":return!0;default:return!1}return null===b||void 0===b?null:!!b},"$data.Blob":function(b){return b},"$data.Object":function(b){return b},"$data.Array":function(b){return b},"$data.ObjectID":function(b){return b&&"string"===typeof b?new $data.mongoDBDriver.ObjectID.createFromHexString((new Buffer(b,
"base64")).toString("ascii")):b},"$data.Geography":function(b){return b?[b.longitude,b.latitude]:b}}}}},{isSupported:{get:function(){return!$data.mongoDBDriver?!1:!0},set:function(){}}});$data.storageProviders.mongoDB.mongoDBProvider.isSupported&&$data.StorageProviderBase.registerProvider("mongoDB",$data.storageProviders.mongoDB.mongoDBProvider);try{"undefined"===typeof navigator&&(navigator=window.navigator=require("navigator"))}catch(err){}
"undefined"===typeof btoa&&(btoa=window.btoa=function(b){return(new Buffer(b,"ascii")).toString("base64")});
